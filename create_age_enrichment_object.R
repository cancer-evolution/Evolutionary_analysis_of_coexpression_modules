...
for(tumour in tumours){
  ...
  for(tissue_type in c("normal", "tumour")){
    if(tissue_type == "tumour" || (tumour %in% tumours_normal & tissue_type == "normal")){
      cluster_names <- names(cluster_assignments[[tissue_type]][[tumour]])
      all_genes <- unique(unname(unlist(cluster_assignments[[tissue_type]][[tumour]])))
      UC_genes_total <- all_genes[all_genes %in% UC_genes]
      MC_genes_total <- all_genes[all_genes %in% MC_genes]
      
      for(cluster_name in cluster_names){
        genes_in_cluster <- cluster_assignments[[tissue_type]][[tumour]][[cluster_name]]
...
#save(age_enrichment, file="age_enrichment.Rdata")
